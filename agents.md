...
- **对外品牌名称：课堂点子铺**（取代过于笼统的 EduNeo Hub；资料夹／技术标识暂不急着改名，等视觉与部署统一时再处理）。
- 核心文案：**老师有点子，课堂有办法。**
...
└── data/
    ...
    └── dskp-index.js                  ← DSKP_INDEX 结构化索引，浏览器直接读取的权威来源（目前只有
                                          Tahun2数学·钱币这一笔，其余科目/年级还没建）
```
...
- 除「Tahun2数学·钱币」外其余科目/年级的 DSKP 结构化索引——要一笔一笔人工核对，不能为了凑数据编
- 缩略图自动生成流程——工具数量增加后再做；目前示例作品缩略图是 Wikimedia Commons 真实照片，真实工具缩略图是手动 Playwright 截图
