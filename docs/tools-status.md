...
> 要改变满意度、已知问题或下一步，请编辑 [tools-status.json](../../teaching-tools/tools-status.json)，然后重新同步。
...
| `tahun2-bc-zonghe` | 已上架 | 待评估 | 尚未用真实班级与访客完成一次成绩写入／读取验证；Vercel preview 已部署，但 Kongsi Idea 团队默认启用 SSO，普通未登录访客会先看到 Vercel 登录页；当前 preview：https://tahun2-bc-zonghe-3z8g2goax-kongsi-idea.vercel.app；朋友提供的生成插画公开发布授权仍需确认 | 老师点击 Hub 入口验收 → 用真实班级与访客验证成绩 → 决定是否调整 preview 保护设定 |
...
| `tahun2-bc-zonghe` | 华文勇者大冒险 | Tahun 2 | 华文 | 待核对 | 已上架 | 待评估 | 0.1.0 | 资料异常 | 有 | 尚未用真实班级与访客完成一次成绩写入／读取验证；Vercel preview 已部署，但 Kongsi Idea 团队默认启用 SSO，普通未登录访客会先看到 Vercel 登录页；当前 preview：https://tahun2-bc-zonghe-3z8g2goax-kongsi-idea.vercel.app；朋友提供的生成插画公开发布授权仍需确认 | 老师点击 Hub 入口验收 → 用真实班级与访客验证成绩 → 决定是否调整 preview 保护设定 | 2026-09-22 |
...
| `tahun3-dst-density` | 浮沉实验室：密度大发现 | Tahun 3 | 科学与科技世界 | 7.1 密度比水大或比水小的物体或材料 — 7.1.1/7.1.2/7.1.3/7.1.4 | 已上架 | 待评估 | 2.0 | 已同步 | 有 | 站④神秘方块排序区的方块图示过小；课本（Unit Ketumpatan）未取得，内容只按 DSKP 摘要；液体分层、加糖未做 | 等课堂反馈；修站④方块图示 | 2026-09-17 |
| `tahun4-bc-bishun` | 四年级写字 | Tahun 4 | 华文 | ⚠️ 引用异常：3.0 — 3.1 | 已上架 | 待评估 | 2.1 | 资料异常 | 有 | 真实滑鼠描写与学校电脑室手感尚未由教师验收；Supabase migration 尚未执行（本机无法解析数据库主机）；Hub 已更新为蓝天草地毛绒场景缩图 | 网络恢复后部署本地 v2.1 并更新 Hub；之后老师打开 https://tahun4-bc-bishun.vercel.app 对照课本第193–194页并实写一字；网络可连接 Supabase 后执行 migration | 2026-09-15 |
...
| 年级 | 已上架工具 |
|---|---|
| Tahun 1 | 马来文 1 个（KVKV音节打地鼠）；华文 6 个（一年级写字、部首大对垒、句型跳跳队、量词大冒险、识字大对决、神奇句子小火车）；数学 1 个（近似值特快车）；科学与科技世界 1 个（磁力创造实验室） |
| Tahun 2 | 华文 2 个（二年级写字、华文勇者大冒险）；数学 6 个（百格图乘法表动画、数字数列大对决、双人数字对决、数序列小探险、苹果果园数学、钱币乐园） |
| Tahun 3 | 华文 2 个（三年级写字、语文课文大PK）；科学与科技世界 1 个（浮沉实验室：密度大发现） |
| Tahun 4 | 英文 1 个（Story Quest · Sentence Workshop）；华文 1 个（四年级写字）；数学 1 个（数学知识大比拼） |
| Tahun 5 | 目前没有已上架工具 |
| Tahun 6 | 目前没有已上架工具 |
