...
| `tahun2-bc-zonghe` | 开发中 | 待评估 | 尚未用真实班级与访客完成一次成绩写入／读取验证；Vercel preview 已部署，但 Kongsi Idea 团队默认启用 SSO，普通未登录访客会先看到 Vercel 登录页；朋友提供的生成插画公开发布授权仍需确认 | 老师点击 preview 验收 → 用真实班级与访客验证成绩 → 决定是否调整 preview 保护设定 → 上架 Hub |
...
| `tahun2-bc-zonghe` | 华文勇者大冒险 | Tahun 2 | 华文 | 待核对 | 开发中 | 待评估 | — | 未上架，不需同步 | 有 | 尚未用真实班级与访客完成一次成绩写入／读取验证；Vercel preview 已部署，但 Kongsi Idea 团队默认启用 SSO，普通未登录访客会先看到 Vercel 登录页；朋友提供的生成插画公开发布授权仍需确认 | 老师点击 preview 验收 → 用真实班级与访客验证成绩 → 决定是否调整 preview 保护设定 → 上架 Hub | 2026-09-22 |
