-- 二年级写字（tahun2-bc-bishun）—— 学生习写生字进度
-- 结构与 tahun1-bc-bishun 完全一致，只换前缀 tahun1_ → tahun2_，
-- 免得两个年级抢同一张表 / 同一个 RPC。
--
...
create table if not exists public.tahun2_bc_bishun_progress (
  id uuid primary key default gen_random_uuid(),
  play_code text,             -- kelasku 班级代码；访客模式是 null
  class_label text not null,  -- 显示用，例如 "2I" 或 "访客"
  name text not null,
...
alter table public.tahun2_bc_bishun_progress enable row level security;

drop policy if exists "anyone can read tahun2_bc_bishun progress" on public.tahun2_bc_bishun_progress;
create policy "anyone can read tahun2_bc_bishun progress"
  on public.tahun2_bc_bishun_progress for select
...
create unique index if not exists tahun2_bc_bishun_progress_key
  on public.tahun2_bc_bishun_progress (play_code, name)
  where play_code is not null;

create or replace function public.submit_tahun2_bc_bishun_progress(
  p_play_code   text,
  p_class_label text,
  p_name        text,
  p_chars_json  jsonb
...
begin
  ...
  if p_play_code is not null then
    select s.id, s.chars_json into v_id, v_old
      from public.tahun2_bc_bishun_progress s
  ...
  if v_id is not null then
    return query
      update public.tahun2_bc_bishun_progress t
    ...
    return query
      insert into public.tahun2_bc_bishun_progress
        (play_code, class_label, name, chars_json, chars_done, attempts)
      values (p_play_code, p_class_label, p_name, v_merged, v_done, 1)
      returning tahun2_bc_bishun_progress.chars_done,
                tahun2_bc_bishun_progress.chars_json,
                tahun2_bc_bishun_progress.attempts;
  end if;
end;
$$;

grant execute on function
  public.submit_tahun2_bc_bishun_progress(text, text, text, jsonb)
  to anon, authenticated;
